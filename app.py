...
# Подключаемся напрямую к серверам через стандартную библиотеку
client = OpenAI(base_url="https://api.sambanova.ai/v1", api_key=API_KEY)

# 📦 ТОЧНЫЕ СИСТЕМНЫЕ ИМЕНА МОДЕЛЕЙ БЕЗ СЛЭШЕЙ
MODELS = {
...
# Поле ввода пользователя со стримингом ответа
    ...
    with st.chat_message("assistant"):
        ...
        try:
            response = client.chat.completions.create(
                ...
                top_p=top_p,
                stream=True  # Вернул плавную печать текста!
            )
